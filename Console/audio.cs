...
namespace PowerSDR
{
    public static class Audio
    {
        ...
        private static bool vac_bypass = false; // // ke9ns: a PTT sets vac_bypass = true
        ...
        unsafe public static int Callback1(void* input, void* output, int frameCount, PA19.PaStreamCallbackTimeInfo* timeInfo, int statusFlags, void* userData)
        {
            ...
            if (!localmox)
            {
                switch (rx1_input_signal)  // ke9ns selected from setup->test->receiver screen
                {
                    ...
                    case SignalSource.SINE_LEFT_ONLY:  // ke9ns add .175
            ...
            if (localmox && (tx_dsp_mode == DSPMode.CWU || tx_dsp_mode == DSPMode.CWL))
            {
                ...
                if (console.setupForm != null) //  ke9ns add
                {
            ...
            {
                if (rx1_output_signal != SignalSource.RADIO) vol = 1.0f;  // ke9ns: do this if you dont select the radio as the output of audio that goes to the speaker
            }
        ...
        unsafe public static int Callback1500(float[] AudioInBuf1, float[] AudioInBuf2, float[] AudioOutBuf1, float[] AudioOutBuf2, uint paFlags)
        {
			...
			Debug.Write(MaxSample(in_l, in_r, frameCount).ToString("f6")+",");
                ...
                if (vac_enabled && vac_output_iq && rb_vacOUT_l != null && rb_vacOUT_r != null && in_l != null && in_r != null)
                {
                    if (!localmox)
                    {
                ...
                if (vac2_enabled && vac2_output_iq && rb_vac2OUT_l != null && rb_vac2OUT_r != null && in_l != null && in_r != null)
                {
                    if (!localmox)
                    {
                        if ((rb_vac2OUT_l.WriteSpace() >= frameCount) && (rb_vac2OUT_r.WriteSpace() >= frameCount))
                ...
                if (vac_enabled && !vac_output_iq && rb_vacIN_l != null && rb_vacIN_r != null && rb_vacOUT_l != null && rb_vacOUT_r != null)
                {
                    if (!localmox)
                    {
                        ...
                        if (console.setupForm.chkVACRXMon.Checked == true) //.318 
                        {
                            ...
                            rb_vacIN_r.ReadPtr(out_l, frameCount);         // leave the out_r1 alone (right channel), it has the receiver audio stream on it
                            Win32.LeaveCriticalSection(cs_vac);

                            ScaleBuffer(out_l, in_l, frameCount, (float)vac_rx_scale); // ke9ns: copy and scale the left channel to out4 VACout
        ...
        unsafe public static int Callback4Port(void* input, void* output, int frameCount, PA19.PaStreamCallbackTimeInfo* timeInfo, int statusFlags, void* userData)
        {
            ...
            if (localmox && (tx_dsp_mode == DSPMode.CWL || tx_dsp_mode == DSPMode.CWU))
            {
                ...
                if (console.setupForm != null) // ke9ns add
                {
                    if (console.setupForm.chkCWDisplay.Checked)
                    {
                        float sum1 = SumBuffer(out_l_ptr2, frameCount); // ke9ns sum up the entire sample to see if anything in it
           ...
           // Check for buffer errors (critical for FireWire stability on Win11) //.323
                ...
                else
                {
                    ...
                    if (rb_vacIN_l.ReadSpace() >= frameCount)  // ke9ns: should come here every cycle through this code on TX (as long as its not bypassed)
                    {
                        if (console.setupForm.chkBoxMixAudio.Checked == true) // ke9ns add
                        {
                            ...
                            Win32.EnterCriticalSection(cs_vac);       // ke9ns used to make sure you have control
                            rb_vacIN_l.ReadPtr(tx_in1_l, frameCount); // ke9ns: copy vacIN ringbuffer and copy it to a temp holder audio pointer to be mixed with mic further down
                            ...
                            ScaleBuffer(tx_in1_r, tx_in1_r, frameCount, (float)vac_preamp); // allow the volume level of the temp holder to be adjusted.
                        }
                        else
                        {
                            Win32.EnterCriticalSection(cs_vac);       // ke9ns used to make sure you have control
                            rb_vacIN_l.ReadPtr(tx_in_l, frameCount);   //send VAC INPUT out the transmitter (no mixing)
                            rb_vacIN_r.ReadPtr(tx_in_r, frameCount);
                            Win32.LeaveCriticalSection(cs_vac);
                    ...
                    if (console.setupForm.chkBoxMixAudio.Checked == false) // ke9ns add
                    {
                        ...
                        ScaleBuffer(tx_in_r, tx_in_r, frameCount, (float)vac_preamp); // this just allows the transmit audio preamp volume adjustment
                    }
                }
                    ...
                    tx_dsp_mode == DSPMode.DIGU)
                {
                    ...
                    else // both VAC1 and VAC2 ON 
                    {
                        if (!vac_bypass)
                        {
                            ...
                            peak = MaxSample(tx_in_l, tx_in_r, frameCount); //   peak = MaxSample(vox_l, vox_r, frameCount);
                ...
                )
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (console.setupForm.chkBoxMixAudio.Checked == true) // ke9ns add  .151 MIX HERE
                        {
                            AddBuffer(tx_in_l, tx_in1_l, frameCount); // the MIC(tx_in_l) + VACIN(tx_in1_l) combined here
            ...
            if (localmox && (tx_dsp_mode == DSPMode.CWL || tx_dsp_mode == DSPMode.CWU))
            {
                ...
                if (console.setupForm != null)// ke9ns add
                {
                    if (console.setupForm.chkCWDisplay.Checked)
                    {
                        float sum1 = SumBuffer(tx_out_l, frameCount); // ke9ns sum up the entire sample to see if anything in it
            ...
            CopyBuffer(out_l1, out_l4, frameCount); // rx1 audio copied to out_l4
            ...
            {
                ClearBuffer(out_l1, frameCount); // receive and normal operation
            ...
            if (!full_duplex) // standard receive/transmit  (in RX out_l1 is cleared because its been copied to out_l4, in TX out_l1 has Mic audio)
            {

                if (!localmox) // RX Mode out_l1 is cleared, copied to out_l4 (ke9ns: only comes here if chkVFOATX is true)
                {
                    ...
                    else if (console.setupForm.checkMicMonRx.Checked == true)  //.318Z
                    {
                        ...
                        {
                            ScaleBuffer(tx_in_l, out_l4, frameCount, (float)monitor_volume); // ke9ns: Mic is just on left channel, so copy to both channels for headphones
                            ScaleBuffer(tx_in_l, out_r4, frameCount, (float)monitor_volume); // tx_in_l
                            Debug.WriteLine("monpre 1");

                        }
                        else  // post
                        {
                            ScaleBuffer(tx_in_l, out_l4, frameCount, (float)monitor_volume); // ke9ns: 
                            ScaleBuffer(tx_in_l, out_r4, frameCount, (float)monitor_volume); // 
                            Debug.WriteLine("monpre 0");
                        }
                    ...
                    if (rx2_enabled)
                    {
                        AddBuffer(out_l4, out_l3, frameCount); // add out_l3 to out_l4  (out_l3 is audio stream from Rx2)
        ...
        unsafe public static int CallbackVAC(void* input, void* output, int frameCount, PA19.PaStreamCallbackTimeInfo* timeInfo, int statusFlags, void* userData)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        Win32.EnterCriticalSection(cs_vac);
                        rb_vacIN_l.WritePtr(in_l_ptr1, frameCount); // ke9ns: VAC1 INPUT stream (i.e. MIC) pointed to in_l_prt1 written into ring buffer rb_vacIN_l
                        rb_vacIN_r.WritePtr(in_l_ptr1, frameCount); //  PC audio stream we want to be able to hear in the PC speaker or Radio speaker if VACRXMON is ON
                        Win32.LeaveCriticalSection(cs_vac);
                    }
                    else
                    {
                ...
                {
                    Win32.EnterCriticalSection(cs_vac);
                    rb_vacOUT_l.ReadPtr(out_l_ptr1, frameCount); //  output audio stream (i.e. Headset) written to ring buffer rb_vacOUT_l
