//=================================================================
// SIOListener.cs ke9ns clone port8 (RX to VFOA TX to VFOB)
//=================================================================
...
namespace PowerSDR
{
    public class SIOListenerII8
    {
        #region Constructor

        public SIOListenerII8(Console c)
        {
            console = c;
            ...
            SDRSerialPort8.serial_rx_event8 += new SerialRXEventHandler(SerialRXEventHandler8);

            if (console.CATEnabled8)  // if CAT is on, fire it up 
            {
                try
                {
                    enableCAT8();
        ...
        public void enableCAT8()
        {
            lock (this)
            {
                if (cat_enabled8) return; // nothing to do already enabled 
                cat_enabled8 = true;
            }
            Debug.WriteLine("==============CAT PORT OPEN");

            int port_num = console.CATPort8;

            SIO8 = new SDRSerialPort8(port_num);
            SIO8.setCommParms(console.CATBaudRate,
                            console.CATParity,
                            console.CATDataBits,
                            console.CATStopBits,
        ...
        public void disableCAT8()
        {
            lock (this)
            {
                if (!cat_enabled8) return; /* nothing to do already disabled */
                cat_enabled8 = false;
            }
        ...
        public SDRSerialPort8 SIO8;
        Console console;
        ASCIIEncoding AE = new ASCIIEncoding();
        private bool Fpass = true;
        private bool cat_enabled8 = false;  // is cat currently enabled by user? 
        ...
        private void SerialRXEventHandler8(object source, SerialRXEvent e)
        {
