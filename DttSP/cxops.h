...
static INLINE COMPLEX Cscl (COMPLEX x, REAL a)
{
	COMPLEX z;
	c_re (z) = c_re (x) * a;
	c_im (z) = c_im (x) * a;
...
static INLINE COMPLEX Csub (COMPLEX x, COMPLEX y)
{
	COMPLEX z;
	c_re (z) = c_re (x) - c_re (y);
	c_im (z) = c_im (x) - c_im (y);
...
static INLINE COMPLEX Cdiv (COMPLEX x, COMPLEX y)
{
...
static INLINE COMPLEX Conjg (COMPLEX z)
{
...
static INLINE COMPLEX Cp2r (COMPLEX z)
{
	return Cmplx ((REAL) (z.re * cos (z.im)), (IMAG) (z.re * sin (z.im)));
}

static INLINE COMPLEX Cr2p (COMPLEX z)
{
	return Cmplx (
