...
//	fm->squelch_strong_timer_hang = (int)(0.02 * samprate); // 3840
//	fm->squelch_weak_timer_hang = (int)(0.15 * samprate); // 28800
//	squelch_threshold_weak:     threshold;	        weak squelch (as though slider to the left)  
//	squelch_threshold_unmute:  threshold * 0.9f;    unmute squelch (as though slider in the middle)
// 	squelch_threshold_strong:  threshold * 0.5f;    strong squelch (as though slider to the right) the value goes down to .005
	...
	for (i = 0; i < CXBsize (fm->squelch_obuf); i++) // = 4096
	{
		...
		{
			if (fm->squelch_filter < fm->squelch_threshold_unmute) //  turn off MUTE if signal is strong enough to unmute
			{
				fm->squelch_muted = FALSE; // 0
    			fm->squelch_strong_timer = fm->squelch_strong_timer_hang; // hysteresis using strong timer = 3840
				fm->squelch_weak_timer = 0; // shut off weak timer
			}
			else
			{
		...
		{
			if(fm->squelch_filter < fm->squelch_threshold_strong)	//strong signal (smaller value)
			{
				fm->squelch_strong_timer = fm->squelch_strong_timer_hang; // = (int)(0.02*samprate); == 3840
				fm->squelch_weak_timer = 0;
	    	}
			else if(fm->squelch_filter < fm->squelch_threshold_weak)	//weak signal
			{
				if (fm->squelch_strong_timer == 0) // check if strong timer has expired either from being MUTED and signal is weak, or UNMUTED and stronger than the weak threshold 
				{
					fm->squelch_weak_timer = fm->squelch_weak_timer_hang;  //  start the hysterisis weak timer == 28800
				}
				else
				{
			...
			else	//very weak signal (no signal) (close to 1.0)
			{
				...
				{
					fm->squelch_muted = TRUE; //  go back into mute
	...
	/*	if (count++ % 96000 == 0)
		{
			fprintf(stderr, "Mute %d, SQLVL: %f, ThresWeak: %3f, thresUnmute: %3f, thresStg: %3f,  \n",
				fm->squelch_muted,
				fm->squelch_filter,
		...
		REAL f_bandwid, int size, COMPLEX * ivec, COMPLEX * ovec, char *tag)
{
	...
	fm->squelch_filter = 1.0f;
	fm->squelch_strong_timer_hang = (int)(0.04 * samprate); // .02 //.336
	fm->squelch_weak_timer_hang = (int)(0.04 *samprate); // .15
...
delFMD (FMD fm)
{
	if (fm)
	{
		...
		del_IIR_HPF_2P(fm->input_HPF3); // ke9ns add for WFM mode
